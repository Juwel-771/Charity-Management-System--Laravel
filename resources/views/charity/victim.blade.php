...
<body>
    ...
    <div class= "row">
        @foreach ($vic as $victim)
        <div class="col-md-8">
            {{-- @foreach ($bloods as $blood) --}}
            <div class="card mb-3">
                <img src="/storage/victim_images/{{$victim->file}}" class="card-img-top">
                <div class="card-body">
                  <h5 class="card-title">{{$victim->ngoName}}</h5>
                  <p class="card-text">{{$victim->description}}"</p>
                  <p class="card-text"><small class="text-muted">Charity Established: {{$victim->ESTD}}</small></p>
                  <a href="{{url('showVictim',$victim->id)}}" class="btn btn-dark">View</a>
                </div>
              </div>
        </div>
        @endforeach
